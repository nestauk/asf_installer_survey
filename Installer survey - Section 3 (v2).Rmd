---
title: "Installer survey - Section 3 (v2)
...
For the same group of installers (company owners, sole traders, and contractors), we now look at how long it took them to become MCS certified. Figure 3.6a shows the distribution.

```{r}
# Merge q24a (company owners) and q24b (sole traders and contractors). Need to turn not asked into NA to merge.
# Duplicate variables x24a and x24b to keep originals
...
data <- data %>% 
  mutate(dummy_no_installs = ifelse(str_detect(x29_answer1, fixed("I haven’t done any installations")) |
                                      str_detect(x29_answer2, fixed("I haven’t done any installations")), 1, 0))
data <- data %>% 
...
# Export to csv
export_to_csv(summary_table, "plot_3.10.csv")
...
ggplot(summary_table, aes(x = factor(dummy_variable, levels = c("It is too expensive",
  ...
  labs(title = "Figure 3.10. Distribution of the reasons why installers who were previously MCS \ncertified choose to remain without it (company owners, sole traders and contractors)", x = "", y = "Count") +
  theme_bw() +
  scale_y_continuous(breaks = seq(0, 10, by = 1)) +
  scale_x_discrete(labels = label_wrap_gen(width = 15))
```
...
data <- data %>% 
  mutate(dummy_no_installs_25a = ifelse(str_detect(x25a_answer1, fixed("I haven’t done any installations")) |
                                      str_detect(x25a_answer2, fixed("I haven’t done any installations")) |
                                      str_detect(x25a_answer3, fixed("I haven’t done any installations")) |
                                      str_detect(x25a_answer4, fixed("I haven’t done any installations")) |
                                      str_detect(x25a_answer5, fixed("I haven’t done any installations")), 1, 0))
data <- data %>% 
...
# Rename variables in the summary_table
new_names <- c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I prefer to have a colleague from another company sign off my installations for MCS certification", "I haven’t done any installations", "Other")

# Create data frame to produce the table
summary_table <- data.frame(dummy_variable = new_names, frequency = as.vector(t(summary_table)))

# Produce plot
ggplot(summary_table, aes(x = factor(dummy_variable, levels = c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I prefer to have a colleague from another company sign off my installations for MCS certification", "I haven’t done any installations", "Other")), y = frequency)) +
  geom_bar(stat = "identity", fill = "blue") +
  geom_text(stat ='identity', aes(label = frequency), vjust = -0.4, size = 3) + 
  labs(title = "Figure 3.12a. Distribution of the reasons why company owners \nchoose not to be MCS certified", x = "", y = "Count") +
  theme_bw() +
  scale_y_continuous(breaks = seq(0, 50, by = 10)) +
...
data <- data %>% 
  mutate(dummy_no_installs_25b = ifelse(str_detect(x25b_answer1, fixed("I haven’t done any installations")) |
                                      str_detect(x25b_answer2, fixed("I haven’t done any installations")) |
                                      str_detect(x25b_answer3, fixed("I haven’t done any installations")) |
                                      str_detect(x25b_answer4, fixed("I haven’t done any installations")) |
                                      str_detect(x25b_answer5, fixed("I haven’t done any installations")), 1, 0))
data <- data %>% 
...
# Export csv
export_to_csv(summary_table, "plot_3.12b.csv")

# Produce plot
ggplot(summary_table, aes(x = factor(dummy_variable, levels = c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I prefer to have a colleague from another company sign off my installations for MCS certification", "I haven’t done any installations", "Other")), y = frequency)) +
  geom_bar(stat = "identity", fill = "blue") +
  geom_text(stat ='identity', aes(label = frequency), vjust = -0.4, size = 3) + 
  labs(title = "Figure 3.12b. Distribution of the reasons why sole traders choose not to be \nMCS certified", x = "", y = "Count") +
  theme_bw() +
...
data <- data %>% 
  mutate(dummy_no_installs_25c = ifelse(str_detect(x25c_answer1, fixed("I haven’t done any installations")) |
                                      str_detect(x25c_answer2, fixed("I haven’t done any installations")) |
                                      str_detect(x25c_answer3, fixed("I haven’t done any installations")) |
                                      str_detect(x25c_answer4, fixed("I haven’t done any installations")) |
                                      str_detect(x25c_answer5, fixed("I haven’t done any installations")), 1, 0))
data <- data %>% 
...
# Export csv
export_to_csv(summary_table, "plot_3.12c.csv")

# Produce plot
ggplot(summary_table, aes(x = factor(dummy_variable, levels = c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I prefer to have a colleague from another company sign off my installations for MCS certification", "I haven’t done any installations", "Other")), y = frequency)) +
  geom_bar(stat = "identity", fill = "blue") +
  geom_text(stat ='identity', aes(label = frequency), vjust = -0.4, size = 3) + 
  labs(title = "Figure 3.12c. Distribution of the reasons why contractors/freelancers choose not \nto be MCS certified", x = "", y = "Count") +
  theme_bw() +
  scale_y_continuous(breaks = seq(0, 15, by = 2)) +
  scale_x_discrete(labels = label_wrap_gen(width = 12))
```
...
data <- data %>% 
  mutate(dummy_no_installs_all = coalesce(dummy_no_installs_25a, dummy_no_installs_25b, dummy_no_installs_25c))
data <- data %>% 
...
# Export csv
export_to_csv(summary_table, "plot_3.12d.csv")

# Produce plot
ggplot(summary_table, aes(x = factor(dummy_variable, levels = c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I prefer to have a colleague from another company sign off my installations for MCS certification", "I haven’t done any installations", "Other")), y = frequency)) +
  geom_bar(stat = "identity", fill = "blue") +
  geom_text(stat ='identity', aes(label = frequency), vjust = -0.2, size = 3) + 
  labs(title = "Figure 3.12d. Distribution of the reasons why installers choose not to be MCS \ncertified (company owners, sole traders and contractors)", x = "", y = "Count") +
  theme_bw() +
...
# Export csv
export_to_csv(summary_long, "plot_3.13.csv")

# Create plot
ggplot(summary_long, aes(x = factor(variable, levels = order_labels), y = count, fill = `Company size`)) +
  geom_bar(stat = "identity") +
  labs(title = "Figure 3.13. Distribution of the reasons why installers choose not to be MCS \ncertified (company owners and sole traders)",
       x = "",
       y = "Count",
       fill = "Company size") +
  theme_bw() +
...
kable(transposed_summary, 
      caption = "Table 3.13. Distribution of the reasons why installers choose not to be MCS \ncertified (company owners and sole traders)",
      align = "c") %>%
  kable_classic(bootstrap_options = c("striped"),
                full_width = T,
                html_font = "Arial") %>%
...
data <- data %>% 
  mutate(dummy_no_installs_merged = coalesce(dummy_no_installs_all, dummy_no_installs))
data <- data %>% 
...
# Rename variables in the summary_table
new_names <- c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I haven't done any installations", "Other")

# Create data frame to produce the table
summary_table <- data.frame(dummy_variable = new_names, frequency = as.vector(t(summary_table)))

# Export csv
export_to_csv(summary_table, "plot_3.14.csv")

# Produce plot
ggplot(summary_table, aes(x = factor(dummy_variable, levels = c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I haven't done any installations", "Other")), y = frequency)) +
  geom_bar(stat = "identity", fill = "blue") +
  geom_text(stat ='identity', aes(label = frequency), vjust = -0.4, size = 3) + 
  labs(title = "Figure 3.14. Distribution of the reasons why installers (excluding employees) \nchoose not to be MCS certified", x = "", y = "Count") +
  theme_bw() +
  scale_y_continuous(breaks = seq(0, 70, by = 10)) +
  scale_x_discrete(labels = label_wrap_gen(width = 12))
```
...
# Rename variables in the summary_table
colnames(summary_table) <- c("Company size", "It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I haven't done any installations", "Other")

# Reshape the data for ggplot
summary_long <- summary_table %>%
  gather(variable, count, -`Company size`)

order_labels <- c("It is too expensive", "There is too much paperwork", "There is too much required training", "My company doesn’t work in retrofit", "I prefer to work through an MCS umbrella scheme", "I prefer to do retrofit outside the MCS scheme", "I haven't done any installations", "Other")

# Export csv
export_to_csv(summary_long, "plot_3.15.csv")

# Create plot
ggplot(summary_long, aes(x = factor(variable, levels = order_labels), y = count, fill = `Company size`)) +
  geom_bar(stat = "identity") +
  labs(title = "Figure 3.15. Distribution of the reasons why installers choose not to be \nMCS certified (company owners and sole traders)",
       x = "",
       y = "Count",
       fill = "Company size") +
  theme_bw() +
